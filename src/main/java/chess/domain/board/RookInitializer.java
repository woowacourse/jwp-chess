...
public final class RookInitializer implements LocationInitializer {
    ...
    private static final List<String> FILES = Arrays.asList("a", "h");
    ...
    public Map<Position, Piece> initialize() {
        ...
        for (final String file : FILES) {
            RANKS_WHITE.forEach(
                rank -> chessBoard.put(new Position(file, rank), new Rook(Color.WHITE, new Position(file, rank))));
            RANKS_BLACK.forEach(
                rank -> chessBoard.put(new Position(file, rank), new Rook(Color.BLACK, new Position(file, rank))));
        }
        return chessBoard;
    }
}
