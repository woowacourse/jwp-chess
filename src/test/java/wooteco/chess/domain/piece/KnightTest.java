...
public class KnightTest {
    ...
    void isMovable() {
        ...
        route.put(Position.of(2, 1), new Knight(Team.WHITE));
        route.put(Position.of(4, 2), new Blank());
        Path path = new Path(route, start, end);

        assertThat(new Knight(Team.WHITE).isMovable(path)).isTrue();
    }

    @Test
    @DisplayName("처음 위치 확인 테스트")
    void isInitialPosition() {
        assertThat(new Knight(Team.WHITE).isInitialPosition(Position.of(1, 2))).isTrue();
    }
}
