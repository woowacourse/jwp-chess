...
class MoveRelationTest {
	...
	void findMultipleStepTraceTest(Position from, Position to, List<Position> expected) {
		List<Position> actual = MoveRelation.findMultipleStepTrace(from, to);
	...
	@ParameterizedTest
	@MethodSource("illegalFromToSet")
	void findMultipleStepTrace_illegal_route__exception_test(Position from, Position to) {
		assertThatThrownBy(() -> MoveRelation.findMultipleStepTrace(from, to))
			.isInstanceOf(IllegalArgumentException.class);
	}

	private static Stream<Arguments> illegalFromToSet() {
		return Stream.of(
