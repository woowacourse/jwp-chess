...
- [x] 체스방 이름과 비밀번호를 입력해 체스방을 만들 수 있다.
- [x] 체스방 목록을 조회할 수 있다. 
- [x] 체스방 목록에서 이름을 클릭해 체스방에 입장할 수 있다. 
- [x] 체스방 목록에서 삭제 버튼을 클릭해 체스방을 삭제할 수 있다. 
  - [x] 비밀번호가 일치해야한다.
...
|--------|--------------------|----------------------|
| GET    | /                  | 메인 페이지               |
| GET    | /room/{id}         | 아이디가 {id}인 방 페이지     |
| GET    | /rooms             | 모든 방 조회              |
| POST   | /rooms             | 새로운 방 생성             |
| GET    | /rooms/{id}        | {id} 방 조회            |
| DELETE | /rooms/{id}        | {id} 방 삭제            |
| GET    | /rooms/{id}/pieces | {id} 방이 소유한 모든 기물 조회 |
| PATCH  | /rooms/{id}/pieces | {id} 방의 기물 위치 변경     |
| GET    | /rooms/{id}/scores | {id} 방의 점수 조회        |
| PATCH  | /rooms/{id}/end    | {id} 방의 게임을 종료       |
