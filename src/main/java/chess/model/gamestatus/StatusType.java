...
public enum StatusType {
    ...
    public static Status createStatus(String statusName, Board board) {
        return Arrays.stream(values())
                .filter(type -> type.name().equals(statusName.toUpperCase()))
                .findFirst()
    ...
    public static StatusType findByStatus(Status status) {
        return Arrays.stream(values())
                .filter(type -> type.hasSameClassName(status))
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("맞는 상태 타입을 찾지 못했습니다."));
    }

    private boolean hasSameClassName(Status status) {
        String className = status.getClass().getSimpleName().toUpperCase();
